...
**TextCNNGLU** emerged as the best-performing architecture with:
- Validation accuracy of 69.40%
- Optimal hyperparameters: LR=0.0005, Dropout=0.2
- Good balance between model capacity and generalization

### Improvements Observed

1. **Data Augmentation**: Increased dataset from 7K to 12K samples and balanced classes, improving performance by ~5-10%
2. **Hyperparameter Tuning**: Grid search identified optimal learning rate and dropout combination
3. **Architecture Design**: GLU and attention pooling improved over baseline CNN approaches

### Challenges Encountered

1. **Overfitting**: Persistent challenge even with regularization, indicating need for more data or stronger regularization
2. **Hyperparameter Sensitivity**: Model performance varied significantly with learning rate and dropout changes
3. **Class Imbalance**: Initial imbalance ratio of 4.71 required careful augmentation strategy
4. **Sequence Length**: Fixed 150-token limit may lose information from longer reviews
5. **Training Instability**: High learning rates (>0.001) caused training failures

### Recommendations for Future Work

1. **Ensemble Methods**: Combine multiple TextCNNGLU models with different initializations
2. **Transfer Learning**: Use pre-trained word embeddings (Word2Vec, GloVe, or BERT embeddings)
3. **Longer Sequences**: Experiment with longer maximum sequence lengths or hierarchical models
4. **Advanced Regularization**: Consider label smoothing, mixup, or other regularization techniques
5. **Architecture Variants**: Test deeper CNNs or hybrid CNN-Transformer architectures
6. **Hyperparameter Optimization**: Use Bayesian optimization or other advanced search methods
