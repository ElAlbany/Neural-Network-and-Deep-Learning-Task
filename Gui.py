...
class PenguinsGUI:
    ...
    def setup_left_frame(self, parent):
        ...
        title_label = ttk.Label(
            scrollable_frame,
            text="Penguins Classification Neural Network",
            font=("Arial", 16, "bold"),
        )
    ...
    def setup_data_section(self, parent):
        ...
        ttk.Button(
            data_frame, text="Load Penguins Dataset", command=self.load_dataset
        ).pack(side=tk.LEFT, padx=(0, 10))

        self.data_status = ttk.Label(
            data_frame, text="No data loaded", foreground="red"
        )
    ...
    def setup_algorithm_section(self, parent):
        ...
        ttk.Radiobutton(
            algo_frame, text="Adaline", variable=self.algorithm_var, value="adaline"
        ).pack(side=tk.LEFT)

    def setup_selection_section(self, parent):
        """Setup feature and class selection section"""
        selection_frame = ttk.LabelFrame(
            parent, text="Feature and Class Selection", padding="10"
        )
        ...
        ttk.Label(feature_frame, text="Feature 1:").grid(
            row=0, column=0, sticky=tk.W, padx=(0, 5)
        )
        self.feature1_var = tk.StringVar()
        self.feature1_combo = ttk.Combobox(
            feature_frame, textvariable=self.feature1_var, state="readonly", width=20
        )
        self.feature1_combo.grid(row=0, column=1, padx=(0, 20), sticky=(tk.W, tk.E))

        ttk.Label(feature_frame, text="Feature 2:").grid(
            row=0, column=2, sticky=tk.W, padx=(0, 5)
        )
        self.feature2_var = tk.StringVar()
        self.feature2_combo = ttk.Combobox(
            feature_frame, textvariable=self.feature2_var, state="readonly", width=20
        )
        ...
        ttk.Label(class_frame, text="Class 1:").grid(
            row=0, column=0, sticky=tk.W, padx=(0, 5)
        )
        self.class1_var = tk.StringVar()
        self.class1_combo = ttk.Combobox(
            class_frame, textvariable=self.class1_var, state="readonly", width=20
        )
        self.class1_combo.grid(row=0, column=1, padx=(0, 20), sticky=(tk.W, tk.E))

        ttk.Label(class_frame, text="Class 2:").grid(
            row=0, column=2, sticky=tk.W, padx=(0, 5)
        )
        self.class2_var = tk.StringVar()
        self.class2_combo = ttk.Combobox(
            class_frame, textvariable=self.class2_var, state="readonly", width=20
        )
    ...
    def setup_parameters_section(self, parent):
        ...
        ttk.Label(param_row1, text="Learning Rate (η):").grid(
            row=0, column=0, sticky=tk.W, padx=(0, 5)
        )
        self.learning_rate_var = tk.DoubleVar(value=0.01)
        ttk.Entry(param_row1, textvariable=self.learning_rate_var, width=10).grid(
            row=0, column=1, padx=(0, 20)
        )

        ttk.Label(param_row1, text="Epochs:").grid(
            row=0, column=2, sticky=tk.W, padx=(0, 5)
        )
        self.epochs_var = tk.IntVar(value=1000)
        ttk.Entry(param_row1, textvariable=self.epochs_var, width=10).grid(
        ...
        ttk.Label(param_row2, text="MSE Threshold:").grid(
            row=0, column=0, sticky=tk.W, padx=(0, 5)
        )
        self.mse_threshold_var = tk.DoubleVar(value=0.001)
        ttk.Entry(param_row2, textvariable=self.mse_threshold_var, width=10).grid(
            row=0, column=1, padx=(0, 20)
        )

        self.bias_var = tk.BooleanVar(value=True)
        ttk.Checkbutton(param_row2, text="Add Bias", variable=self.bias_var).grid(
            row=0, column=2, padx=(20, 0)
        )

        ttk.Label(param_row2, text="Random State:").grid(
            row=0, column=3, sticky=tk.W, padx=(20, 5)
        )
        self.random_state_var = tk.IntVar(value=42)
        ttk.Entry(param_row2, textvariable=self.random_state_var, width=10).grid(
            row=0, column=4
        )
    ...
    def setup_control_section(self, parent):
        ...
        ttk.Button(button_row1, text="Train Model", command=self.train_model).pack(
            side=tk.LEFT, padx=(0, 10)
        )
        ttk.Button(button_row1, text="Test Model", command=self.test_model).pack(
            side=tk.LEFT, padx=(0, 10)
        )
        ttk.Button(
            button_row1, text="Single Prediction", command=self.single_prediction
        ).pack(side=tk.LEFT, padx=(0, 10))

        button_row2 = ttk.Frame(control_frame)
        button_row2.pack(fill=tk.X, pady=5)

        ttk.Button(button_row2, text="Plot Results", command=self.plot_results).pack(
            side=tk.LEFT, padx=(0, 10)
        )
        ttk.Button(button_row2, text="Clear Plot", command=self.clear_plot).pack(
            side=tk.LEFT, padx=(0, 10)
        )
        ttk.Button(button_row2, text="Clear Results", command=self.clear_results).pack(
            side=tk.LEFT, padx=(0, 10)
        )
        ttk.Button(button_row2, text="Clear All", command=self.clear_all).pack(
            side=tk.LEFT
        )
    ...
    def setup_visualization_tab(self, parent):
        ...
        h_scrollbar = ttk.Scrollbar(
            plot_frame, orient="horizontal", command=self.viz_canvas.xview
        )
        self.viz_canvas.configure(xscrollcommand=h_scrollbar.set)

        v_scrollbar = ttk.Scrollbar(
            plot_frame, orient="vertical", command=self.viz_canvas.yview
        )
        ...
        self.plot_status = ttk.Label(
            viz_main_frame, text="No plot generated", foreground="gray"
        )
    ...
    def train_model(self):
        ...
        self.X_train, self.X_test, self.y_train, self.y_test, filtered_data = (
            self.data_processor.prepare_data(
                self.feature1_var.get(),
                self.feature2_var.get(),
                self.class1_var.get(),
                self.class2_var.get(),
        ...
        if algorithm == "perceptron":
            self.current_model = Perceptron(
                learning_rate=learning_rate, n_iterations=epochs, add_bias=add_bias
            )
        else:
        ...
        self.log_result(
            f"Features: {self.feature1_var.get()}, {self.feature2_var.get()}"
        )
        self.log_result(
            f"Classes: {self.class1_var.get()} (-1) vs {self.class2_var.get()} (1)"
        )
        ...
        if algorithm == "perceptron":
            ...
            self.log_result(
                f"Converged: {'Yes' if self.current_model.converged else 'No'}"
            )
        else:
    ...
    def test_model(self):
        ...
        try:
            ...
            confusion_matrix, accuracy = self.nn_manager.create_confusion_matrix(
                self.y_test, y_pred
            )
            ...
            self.log_result("\nConfusion Matrix:")
            self.log_result(
                f"                Predicted {self.current_classes[0]:<12} | Predicted {self.current_classes[1]:<12}"
            )
            self.log_result(
                f"Actual {self.current_classes[0]:<10} {confusion_matrix[0, 0]:>10} | {confusion_matrix[0, 1]:>10}"
            )
            self.log_result(
                f"Actual {self.current_classes[1]:<10} {confusion_matrix[1, 0]:>10} | {confusion_matrix[1, 1]:>10}"
            )
    ...
    def plot_results(self):
        ...
        try:
            ...
            self.plot_status.config(
                text="Plot generated successfully - Use scrollbars to navigate",
                foreground="green",
            )
    ...
    def single_prediction(self):
        ...
        try:
            ...
            messagebox.showerror(
                "Prediction Error", f"Failed to make prediction: {str(e)}"
            )
    ...
    def create_prediction_dialog(self):
        ...
        ttk.Label(
            dialog, text=f"Single Sample Prediction", font=("Arial", 12, "bold")
        ).pack(pady=10)
        ...
        ttk.Label(feature1_frame, text=f"{self.current_features[0]}:").pack(
            side=tk.LEFT
        )
        feature1_var = tk.DoubleVar(value=40.0)
        ...
        ttk.Label(feature2_frame, text=f"{self.current_features[1]}:").pack(
            side=tk.LEFT
        )
        feature2_var = tk.DoubleVar(value=18.0)
        ...
        true_class_combo = ttk.Combobox(
            true_class_frame, textvariable=true_class_var, state="readonly", width=15
        )
        true_class_combo["values"] = (
            "Unknown",
        ...
        def predict():
            try:
                ...
                predicted_class = (
                    self.current_classes[1]
                    if prediction == 1
                    else self.current_classes[0]
                )
                true_class = true_class_var.get()

                result_message = f"Input Features:\n"
                result_message += (
                    f"  • {self.current_features[0]}: {feature1_var.get()}\n"
                )
                result_message += (
                ...
                if true_class != "Unknown":
                    ...
                    if is_correct:
                        result_text.tag_configure(
                            "correct", foreground="green", font=("Arial", 10, "bold")
                        )
                    else:
                        result_text.tag_configure(
                            "wrong", foreground="red", font=("Arial", 10, "bold")
                        )
                ...
                if true_class != "Unknown":
                    ...
                    if is_correct:
                        result_text.tag_add(
                            "correct",
                            f"{start_idx}+2lines",
                            f"{start_idx}+2lines+7chars",
                        )
                    else:
                        result_text.tag_add(
                            "wrong", f"{start_idx}+2lines", f"{start_idx}+2lines+6chars"
                        )
                ...
                self.log_result(f"\nSingle Prediction:")
                self.log_result(
                    f"  Features: {self.current_features[0]}={feature1_var.get()}, "
                    f"{self.current_features[1]}={feature2_var.get()}"
                )
        ...
        ttk.Button(button_frame, text="Predict", command=predict).pack(
            side=tk.LEFT, padx=(0, 10)
        )
        ttk.Button(button_frame, text="Clear", command=clear_inputs).pack(
            side=tk.LEFT, padx=(0, 10)
        )
    ...
    def load_dataset(self):
        ...
        if success:
            ...
            self.log_result(
                f"Available classes: {', '.join(self.data_processor.classes)}"
            )
            self.log_result(
                "NOTE: Features are now automatically scaled for better performance!"
            )
        else:
