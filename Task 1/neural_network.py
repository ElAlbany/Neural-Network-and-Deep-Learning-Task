...
class Adaline:
    def __init__(
        self, learning_rate=0.01, n_iterations=1000, mse_threshold=0.001, add_bias=True
    ):
        self.learning_rate = learning_rate
        self.n_iterations = n_iterations
    ...
    def fit(self, X, y):
        ...
        for iteration in range(self.n_iterations):
            ...
            if mse <= self.mse_threshold:
                ...
                iteration > 10
                and abs(self.mse_history[-1] - self.mse_history[-2]) < 1e-8
            ):
                self.converged = True
                break
...
class NeuralNetworkManager:
    ...
    def plot_decision_boundary(self, X, y, model, feature_names, class_names):
        ...
        if isinstance(model, Perceptron) and model.errors:
            ...
            ax2.plot(
                range(1, len(model.mse_history) + 1),
                model.mse_history,
                marker="o",
                color="orange",
                linewidth=2,
